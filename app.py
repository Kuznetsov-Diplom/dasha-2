...
def create_vector_bar_plot(vector: list, title: str = "39-мерный вектор признаков") -> go.Figure:
    fig = go.Figure()
    colors = ["#FF6B6B" if v > 0.7 else "#4ECDC4" for v in vector]
    fig.add_trace(go.Bar(x=[f"F{i+1}" for i in range(39)], y=vector, marker_color=colors, text=[f"{v:.2f}" for v in vector], textposition="outside", textfont=dict(size=9)))
...
def create_correlation_heatmap(vectors: list, labels: list) -> go.Figure:
    ...
    fig = go.Figure(data=go.Heatmap(z=corr, x=labels, y=labels, colorscale="RdYlBu_r", zmin=-1, zmax=1, colorbar=dict(title="Корреляция")))
...
def process_correlation(files, use_rasta):
    ...
    for i, v in enumerate(vectors):
        fig_lines.add_trace(go.Scatter(x=list(range(39)), y=v, mode="lines+markers", name=labels[i], line=dict(width=1.5), opacity=0.7))
    fig_lines.add_trace(go.Scatter(x=list(range(39)), y=mean_vec, mode="lines", name="Средний эталон", line=dict(color="black", width=3, dash="dash")))
...
def run_gost_mass_test(num_speakers, phrases_per_speaker, use_rasta):
    ...
    for i, v in enumerate(all_vectors[:min(12, len(all_vectors))]):
        fig_lines.add_trace(go.Scatter(x=list(range(39)), y=v, mode="lines", name=speaker_labels[i], opacity=0.6))
